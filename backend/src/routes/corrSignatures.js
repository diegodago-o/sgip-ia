...
// ── Notificaciones al CREADOR del flujo (correspondencia) ─────────────────────

function emailCreatorStartedCorr({ creator, corr, project, signers }) {
  const typeLabel = TYPE_LABEL[corr.correspondence_type] || 'COMUNICACIÓN';
  ...
  <div style="background:white;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.1);">
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f0f7ff;border-left:4px solid #1B5FAA;border-radius:8px;padding:16px 20px;margin:20px 0;">
        ...
        <p style="margin:0;font-size:16px;font-weight:bold;color:#1B5FAA;">${corr.subject}</p>
        <p style="margin:4px 0 0;font-size:13px;color:#374151;">${typeLabel} · ${corr.consecutive_code} · Proyecto: <strong>${project.name} (${project.code})</strong></p>
      </div>
...
function emailCreatorProgressCorr({ creator, signer, allSigners, corr, project }) {
  ...
  <div style="background:white;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.1);">
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f0fdf4;border-left:4px solid #059669;border-radius:8px;padding:16px 20px;margin:16px 0;">
        ...
        <p style="margin:6px 0 0;font-size:13px;color:#374151;">Documento: <strong>${corr.subject}</strong> · ${project.name}</p>
      </div>
...
router.post('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (req.user?.email && emailCfg) {
      await trySendMail(emailCfg, {
        to: req.user.email,
        subject: `🔔 Flujo de firmas iniciado: ${corr.subject} — ${project.name}`,
        html: emailCreatorStartedCorr({ creator: req.user, corr, project, signers: signerRecords }),
      });
    }
...
publicRouter.post('/:token/firmar', async (req, res) => {
  ...
  try {
    ...
    if (pending.length === 0) {
      ...
      if (creatorC?.email) {
        ...
        if (!signerEmails.has(creatorC.email.toLowerCase())) {
          await trySendMail(emailCfg, {
            to: creatorC.email,
            subject: `✅ Documento firmado completo: ${corr.subject} — ${project.name}`,
            html: completedHtml,
            attachments: pdfAttachment,
          });
        }
    ...
    if (creatorP?.email) {
      ...
      await trySendMail(emailCfg, {
        to: creatorP.email,
        subject: `🔔 ${signer.signer_name} firmó (${updatedSigners.filter(s=>s.status==='signed').length}/${updatedSigners.length}): ${corr.subject}`,
        html: emailCreatorProgressCorr({ creator: creatorP, signer, allSigners: updatedSigners, corr, project }),
      });
    }
