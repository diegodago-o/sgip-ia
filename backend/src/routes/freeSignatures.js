...
// ── Notificaciones al CREADOR del flujo (firma libre) ────────────────────────

function emailCreatorStartedFree({ creator, request, project, signers }) {
...
<div style="max-width:640px;margin:32px auto;padding:0 16px;">
  <div style="background:white;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.1);">
    ...
    <div style="padding:32px;">
      ...
      <p style="color:#374151;font-size:14px;line-height:1.6;">Has iniciado un proceso de firma digital para el siguiente documento:</p>
      <div style="background:#f0f7ff;border-left:4px solid #1B5FAA;border-radius:8px;padding:16px 20px;margin:20px 0;">
        ...
        <p style="margin:0;font-size:16px;font-weight:bold;color:#1B5FAA;">${request.title}</p>
        <p style="margin:4px 0 0;font-size:13px;color:#374151;">${request.file_name} · Proyecto: <strong>${project.name} (${project.code})</strong></p>
      </div>
      ...
      <p style="font-size:13px;color:#64748b;background:#f8fafc;padding:12px 16px;border-radius:8px;">
        Te notificaremos cada vez que un firmante complete su firma. Al finalizar recibirás copia del documento firmado con página de auditoría.
      </p>
      <hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0;">
...
function emailCreatorProgressFree({ creator, signer, allSigners, request, project }) {
  ...
  <div style="background:white;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.1);">
    ...
    <div style="padding:32px;">
      ...
      <div style="background:#f0fdf4;border-left:4px solid #059669;border-radius:8px;padding:16px 20px;margin:16px 0;">
        ...
        <p style="margin:6px 0 0;font-size:13px;color:#374151;">Documento: <strong>${request.title}</strong> · ${project.name}</p>
      </div>
...
async function notifyNextSigner(requestId) {
  ...
  if (allSigned) {
    ...
    try {
      const [[creatorC]] = await pool.execute('SELECT id, email, full_name FROM users WHERE id=?', [request.created_by]);
      if (creatorC?.email) {
        const signerEmails = new Set(signers.map(s => s.signer_email.toLowerCase()));
        if (!signerEmails.has(creatorC.email.toLowerCase())) {
          const mailOpts = {
            to: creatorC.email,
            subject: `✅ Documento firmado completo: ${request.title} — ${project.name}`,
            html,
          };
          if (signedPdf) {
            mailOpts.attachments = [{ filename: `firmado_${request.file_name}`, content: signedPdf, contentType: 'application/pdf' }];
  ...
  try {
    ...
    if (creatorP?.email) {
      const lastSigned = signers.filter(s => s.status === 'signed').slice(-1)[0];
      if (lastSigned) {
        await trySendMail(emailCfg, {
          to: creatorP.email,
          subject: `🔔 ${lastSigned.signer_name} firmó (${signers.filter(s=>s.status==='signed').length}/${signers.length}): ${request.title}`,
...
authRouter.post('/', authenticate, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    try {
      const [[proj]] = await pool.execute('SELECT name, code FROM projects WHERE id=?', [projectId]);
      const emailCfg = await loadEmailConfig();
      if (req.user?.email && emailCfg && proj) {
        const [allFSigners] = await pool.execute(
          'SELECT * FROM free_signature_signers WHERE request_id=? ORDER BY sign_order', [requestId]
        );
        await trySendMail(emailCfg, {
          to: req.user.email,
          subject: `🔔 Flujo de firmas iniciado: ${title.trim()} — ${proj.name}`,
          html: emailCreatorStartedFree({
            creator: req.user,
            request: { title: title.trim(), file_name: file.originalname },
