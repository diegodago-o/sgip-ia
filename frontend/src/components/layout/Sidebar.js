...
export default function Sidebar() {
  ...
  const linkClasses = (isActive) =>
    `group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
      isActive ? 'bg-brand-600 text-white shadow-md shadow-brand-600/20' : 'text-brand-200 hover:bg-white/10 hover:text-white'
    }`;

  return (
    <aside className={`flex flex-col bg-brand-900 text-white transition-all duration-300 ease-out shadow-sidebar ${collapsed ? 'w-[72px]' : 'w-[260px]'}`}>
      <div className={`flex items-center h-16 px-4 border-b border-white/10 ${collapsed ? 'justify-center' : 'justify-between'}`}>
        <div className="flex items-center gap-2.5 overflow-hidden">
          <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0">
            <Cpu className="w-4.5 h-4.5 text-brand-300" />
          </div>
          {!collapsed && <span className="font-display font-bold text-base tracking-tight whitespace-nowrap animate-fade-in">SGIP-IA</span>}
        </div>
        {!collapsed && (
          <button onClick={() => setCollapsed(true)} className="w-7 h-7 rounded-md bg-white/5 hover:bg-white/10 flex items-center justify-center">
            <ChevronLeft className="w-4 h-4 text-brand-300" />
          </button>
        )}
      </div>

      {collapsed && (
        <button onClick={() => setCollapsed(false)} className="w-full flex justify-center py-3 text-brand-400 hover:text-white transition-colors">
          <ChevronRight className="w-4 h-4" />
        </button>
      )}

      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {!collapsed && <p className="px-3 mb-2 text-[10px] uppercase tracking-widest text-brand-500 font-semibold">Módulos</p>}

        {visibleModules.map((item) => {
          const Icon = item.icon;
          const isActive = location.pathname === item.to;
          return (
            <NavLink key={item.to} to={item.to} className={() => linkClasses(isActive)} title={collapsed ? item.label : undefined}>
              <Icon className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? 'text-white' : 'text-brand-400 group-hover:text-brand-200'}`} />
              {!collapsed && <span className="flex-1 truncate">{item.label}</span>}
              {!collapsed && item.soon && <span className="px-1.5 py-0.5 text-[10px] rounded bg-white/10 text-brand-400 font-mono">Soon</span>}
            </NavLink>
          );
        })}
      ...
      <div className={`border-t border-white/10 p-3 ${collapsed ? 'flex justify-center' : ''}`}>
        {collapsed ? (
          <button onClick={logout} className="w-9 h-9 rounded-lg bg-white/5 hover:bg-red-500/20 flex items-center justify-center" title="Cerrar sesión">
            <LogOut className="w-4 h-4 text-brand-400 hover:text-red-400" />
          </button>
        ) : (
          <div className="flex items-center gap-3 animate-fade-in">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-brand-400 truncate">{ROLE_LABELS[role] || role}</p>
            </div>
            <button onClick={logout} className="w-8 h-8 rounded-lg hover:bg-red-500/20 flex items-center justify-center" title="Cerrar sesión">
              <LogOut className="w-4 h-4 text-brand-400 hover:text-red-400" />
            </button>
          </div>
        )}
